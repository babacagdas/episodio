...
export function useWatchlist(enabled = true) {
  ...
  const toggle = useCallback(async (item: WatchlistItem) => {
    ...
    // ⚡ OPTIMISTIC UI: 0ms anında yerel liste güncellemesi
    setWatchlist(prev =>
    ...
    try {
      ...
      if (user) {
        if (exists) {
          const { error } = await supabase.from('watchlist').delete().eq('user_id', user.id).eq('show_id', item.id);
          if (error) {
      ...
      setWatchlist(prev =>
        exists ? [...prev, item] : prev.filter(i => i.id !== item.id)
      );
    }
  }, [watchlist]);
