...
export function BottomNav() {
  ...
  return (
    <nav className="bg-[#1A1A1A]/70 backdrop-blur-2xl fixed bottom-0 left-0 w-full z-50 flex justify-around items-center px-4 pb-6 pt-3 border-t border-white/10 shadow-[0_-10px_30px_rgba(0,0,0,0.5)] rounded-t-lg md:hidden">
      {navItems.map(({ href, icon, label }) => {
        ...
        return (
          ...
          >
            ...
            <span className="font-['Be_Vietnam_Pro'] text-[10px] font-medium uppercase tracking-widest">{label}</span>
          </Link>
        );
      })}
    </nav>
